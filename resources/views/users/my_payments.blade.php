  ...
  <body>
    ...
    <div class="sl-mainpanel">
      <nav class="breadcrumb sl-breadcrumb">
        <a class="breadcrumb-item" href="{{ route('dashboard') }}">{{ $site->site_name }}</a>
        {{-- <a class="breadcrumb-item" href="index.html">Tables</a> --}}
        <span class="breadcrumb-item active">My Payments History</span>
      </nav>
      ...
      <div class="sl-pagebody">
        ...
        <div class="card pd-20 pd-sm-25 mg-t-20">     
            ...
            <div class="table-wrapper">
             @if (count($Applications) > 0) <table id="datatable1" class="table table-hover table-striped table-responsive display responsive nowrap table-bordered table-indigo disabled">
              <thead>
                <tr>
              ...
              <tbody>
                @foreach ($Applications as $k => $Application)
                  ...
                  @endphp --}}
                <tr>
                  <td data-toggle="tooltip" data-placement="top" title="Tab to see more info">{{ $Application->sch_name }} <span class="d-md-none d-sm-block" style="font-weight: 600">+</span></td>
                  {{-- <td>{{ $Application->sch_name }}</td> --}}
                  <td>N{{ number_format($Application->payable) }}.</td>
                  <td>{!! date('D, d-M-y', strtotime($Application->created_at)) !!}</td>
                  <td>
                    @if($Application->created_at == $Application->updated_at)
                    {{"Not yet modified"}}
                    @else
                    {!! date('D, d-M-y', strtotime($Application->updated_at)) !!}
                    @endif
                  </td>
            ...
            <div class="text-center" style="padding: 20px 0 20px 0">
              All your payments will Appear here!</div> 
             @endif
              ...
              <div class="card-footer tx-12 pd-y-1 bg-transparent bd-t bd-gray-200">
                <a href="{{ route('Applications.index') }}"><i class="fa fa-angle-down mg-r-5"></i>See Programs for Application</a>
              </div>
